...
static void hisi_l3c_pmu_write_counter(const struct hisi_pmu *l3c_pmu,
...
static void hisi_l3c_pmu_write_evtype(const struct hisi_pmu *l3c_pmu, int idx,
                                      u32 type)
{
...
static void hisi_l3c_pmu_start_counters(const struct hisi_pmu *l3c_pmu)
{
    u32 val;
    if (l3c_pmu == NULL) {
        return;
...
static void hisi_l3c_pmu_stop_counters(const struct hisi_pmu *l3c_pmu)
{
    u32 val;
    if (l3c_pmu == NULL) {
        return;
...
static void hisi_l3c_pmu_enable_counter(const struct hisi_pmu *l3c_pmu,
                                        const struct hw_perf_event *hwc)
{
    u32 val;

    if (l3c_pmu == NULL || hwc == NULL) {
        return;
    }
...
static void hisi_l3c_pmu_disable_counter(const struct hisi_pmu *l3c_pmu,
                                         const struct hw_perf_event *hwc)
{
    u32 val;
    if (l3c_pmu == NULL || hwc == NULL) {
...
static void hisi_l3c_pmu_enable_counter_int(const struct hisi_pmu *l3c_pmu,
                                            const struct hw_perf_event *hwc)
{
    u32 val;
    if (l3c_pmu == NULL || hwc == NULL) {
...
static void hisi_l3c_pmu_disable_counter_int(const struct hisi_pmu *l3c_pmu,
                                             const struct hw_perf_event *hwc)
{
    u32 val;
    if (l3c_pmu == NULL || hwc == NULL) {
...
static int hisi_l3c_pmu_probe(struct platform_device *pdev)
{
    ...
    if (ret) {
        ...
        if (cpuhp_state_remove_instance_nocalls(CPUHP_AP_PERF_ARM_HISI_L3_ONLINE, &l3c_pmu->node) == -EINVAL) {
            dev_err(l3c_pmu->dev, "L3C PMU remove instance failed!\n");
        }

        if (irq_set_affinity_hint(l3c_pmu->irq, NULL) == -EINVAL) {
            dev_err(l3c_pmu->dev, "L3C PMU set affinity failed!\n");
        }
    }
...
static int hisi_l3c_pmu_remove(struct platform_device *pdev)
{
    ...
    l3c_pmu = platform_get_drvdata(pdev);
    if (l3c_pmu == NULL) {
        return -EINVAL;
    }
