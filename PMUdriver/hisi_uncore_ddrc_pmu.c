                                      ...
                                      struct hw_perf_event *hwc)
{
    ...
    if (hwc == NULL || ddrc_pmu == NULL) {
        return 0;
    }

    /* Use event code as counter index */
    idx = GET_DDRC_EVENTID(hwc);
    ...
    offset = hisi_ddrc_pmu_get_counter_offset(idx);
    if (offset == EINVAL) {
        return 0;
    }
    return readl(ddrc_pmu->base + offset);
}

static void hisi_ddrc_pmu_write_counter(const struct hisi_pmu *ddrc_pmu,
                                        const struct hw_perf_event *hwc, u64 val)
{
    u32 idx;
    u32 offset;
    ...
    writel((u32)val, ddrc_pmu->base + offset);
}
 ...
 */
static void hisi_ddrc_pmu_write_evtype(const struct hisi_pmu *hha_pmu, int idx,
                                       u32 type)
{
}

static void hisi_ddrc_pmu_start_counters(const struct hisi_pmu *ddrc_pmu)
{
    u32 val;
...
static void hisi_ddrc_pmu_stop_counters(const struct hisi_pmu *ddrc_pmu)
{
    u32 val;
...
static void hisi_ddrc_pmu_enable_counter(const struct hisi_pmu *ddrc_pmu,
...
static void hisi_ddrc_pmu_disable_counter(const struct hisi_pmu *ddrc_pmu,
                                          const struct hw_perf_event *hwc)
{
    u32 val;
...
{
    struct hisi_pmu *ddrc_pmu = NULL;
    unsigned long *used_mask = NULL;
    struct hw_perf_event *hwc = NULL;
    int idx;

    if (event == NULL || event->pmu == NULL) {
        return -EAGAIN;
    }

    ddrc_pmu = to_hisi_pmu(event->pmu);
    if (ddrc_pmu == NULL) {
        return -EAGAIN;
    }
    used_mask = ddrc_pmu->pmu_events.used_mask;
    hwc = &event->hw;
    if (hwc == NULL) {
        return -EAGAIN;
    }
    ...
    if (used_mask == NULL || test_bit(idx, used_mask))
        return -EAGAIN;
...
static void hisi_ddrc_pmu_enable_counter_int(const struct hisi_pmu *ddrc_pmu,
                                             const struct hw_perf_event *hwc)
{
    u32 val;
...
static void hisi_ddrc_pmu_disable_counter_int(const struct hisi_pmu *ddrc_pmu,
                                              const struct hw_perf_event *hwc)
{
    u32 val;
...
static int hisi_ddrc_pmu_probe(struct platform_device *pdev)
{
    ...
    if (ret) {
        ...
        if (cpuhp_state_remove_instance_nocalls(CPUHP_AP_PERF_ARM_HISI_DDRC_ONLINE, &ddrc_pmu->node) == -EINVAL) {
            dev_err(ddrc_pmu->dev, "DDRC PMU remove instance failed!\n");
        }

        if (irq_set_affinity_hint(ddrc_pmu->irq, NULL) == -EINVAL) {
            dev_err(ddrc_pmu->dev, "DDRC PMU set affinity failed!\n");
        }
    }
...
static int hisi_ddrc_pmu_remove(struct platform_device *pdev)
{
    ...
    ddrc_pmu = platform_get_drvdata(pdev);
    if (ddrc_pmu == NULL) {
        return -EINVAL;
    }
