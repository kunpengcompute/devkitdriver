...
static void hisi_hha_pmu_write_counter(const struct hisi_pmu *hha_pmu,
...
static void hisi_hha_pmu_write_evtype(const struct hisi_pmu *hha_pmu, int idx,
                                      u32 type)
{
...
static void hisi_hha_pmu_start_counters(const struct hisi_pmu *hha_pmu)
{
    u32 val;
...
static void hisi_hha_pmu_stop_counters(const struct hisi_pmu *hha_pmu)
{
    u32 val;
...
static void hisi_hha_pmu_enable_counter(const struct hisi_pmu *hha_pmu,
                                        const struct hw_perf_event *hwc)
{
    u32 val;

    if (hha_pmu == NULL || hwc == NULL) {
        return;
    }
...
static void hisi_hha_pmu_disable_counter(const struct hisi_pmu *hha_pmu,
                                         const struct hw_perf_event *hwc)
{
    u32 val;
...
static void hisi_hha_pmu_enable_counter_int(const struct hisi_pmu *hha_pmu,
                                            const struct hw_perf_event *hwc)
{
    u32 val;
...
static void hisi_hha_pmu_disable_counter_int(const struct hisi_pmu *hha_pmu,
                                             const struct hw_perf_event *hwc)
{
    u32 val;
...
static int hisi_hha_pmu_probe(struct platform_device *pdev)
{
    ...
    if (ret) {
        ...
        if (cpuhp_state_remove_instance_nocalls(CPUHP_AP_PERF_ARM_HISI_HHA_ONLINE, &hha_pmu->node) == -EINVAL) {
            dev_err(hha_pmu->dev, "HHA PMU remove instance failed!\n");
        }

        if (irq_set_affinity_hint(hha_pmu->irq, NULL) == -EINVAL) {
            dev_err(hha_pmu->dev, "HHA PMU set affinity failed!\n");
        }
    }
    return ret;
}
...
static int hisi_hha_pmu_remove(struct platform_device *pdev)
{
    ...
    hha_pmu = platform_get_drvdata(pdev);
    if (hha_pmu == NULL) {
        return -EINVAL;
    }
