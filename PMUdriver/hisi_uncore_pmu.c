...
void (*perf_event_update_userpage_pmu)(struct perf_event *event);
...
ssize_t hisi_cpumask_sysfs_show(struct device *dev,
                                struct device_attribute *attr, char *buf)
{
    struct hisi_pmu *hisi_pmu = NULL;
    if (dev == NULL || buf == NULL) {
        return -1;
    }
    hisi_pmu = to_hisi_pmu(dev_get_drvdata(dev));
    if (hisi_pmu == NULL) {
        return -1;
    }
...
{
    struct perf_event *sibling = NULL, *leader = NULL;
    struct hisi_pmu *hisi_pmu = NULL;
    ...
    list_for_each_entry(sibling, &leader->sibling_list, sibling_list) {
        if (is_software_event(sibling))
            continue;
        if (sibling == NULL || sibling->pmu != event->pmu)
            return false;
...
int hisi_uncore_pmu_counter_valid(const struct hisi_pmu *hisi_pmu, int idx)
{
    if (hisi_pmu == NULL) {
        return 0;
    }
...
int hisi_uncore_pmu_get_event_idx(struct perf_event *event)
{
    ...
    used_mask = hisi_pmu->pmu_events.used_mask;
    if (used_mask == NULL) {
        return -EAGAIN;
    }
    num_counters = hisi_pmu->num_counters;
...
static void hisi_uncore_pmu_enable_event(struct perf_event *event)
{
    ...
    if (hisi_pmu == NULL || hwc == NULL || hisi_pmu->ops == NULL) {
        return;
    }
...
void hisi_uncore_pmu_start(struct perf_event *event, int flags)
{
    ...
    if (flags & PERF_EF_RELOAD) {
        ...
        if (hisi_pmu == NULL || hisi_pmu->ops == NULL) {
            return;
        }
    ...
    perf_event_update_userpage_pmu = (void *)kallsyms_lookup_name("perf_event_update_userpage");
    if (perf_event_update_userpage_pmu == NULL) {
        return;
    }
    perf_event_update_userpage_pmu(event);
}
...
int hisi_uncore_pmu_add(struct perf_event *event, int flags)
{
    ...
    hisi_pmu = to_hisi_pmu(event->pmu);
    hwc = &event->hw;
    if (hwc == NULL || hisi_pmu == NULL || hisi_pmu->ops == NULL) {
        return -EINVAL;
    }
...
void hisi_uncore_pmu_enable(struct pmu *pmu)
{
    ...
    hisi_pmu = to_hisi_pmu(pmu);
    if (hisi_pmu == NULL) {
        return;
    }
    enabled = bitmap_weight(hisi_pmu->pmu_events.used_mask,
                            hisi_pmu->num_counters);
    if (!enabled || hisi_pmu->ops == NULL) {
        return;
    }
...
{
    struct hisi_pmu *hisi_pmu = hlist_entry_safe(node, struct hisi_pmu, node);
...
int hisi_uncore_pmu_offline_cpu(unsigned int cpu, struct hlist_node *node)
{
    ...
    if (hisi_pmu == NULL || !cpumask_test_and_clear_cpu(cpu, &hisi_pmu->associated_cpus))
        return 0;
