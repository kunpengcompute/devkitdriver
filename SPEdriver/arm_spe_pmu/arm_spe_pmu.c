...
#define SPE_EXT_ATTR_ENTRY(_name, _func, _var) \
    &((struct dev_ext_attribute[]){{__ATTR(_name, S_IRUGO, _func, NULL), (void*)_var}})[0].attr.attr

#define SPE_CAP_EXT_ATTR_ENTRY(_name, _var) SPE_EXT_ATTR_ENTRY(_name, arm_spe_pmu_cap_show, _var)
...
#define _GEN_PMU_FORMAT_ATTR(cfg, lo, hi) __GEN_PMU_FORMAT_ATTR(cfg, lo, hi)

#define GEN_PMU_FORMAT_ATTR(name) \
    PMU_FORMAT_ATTR(              \
        name, _GEN_PMU_FORMAT_ATTR(ATTR_CFG_FLD_##name##_CFG, ATTR_CFG_FLD_##name##_LO, ATTR_CFG_FLD_##name##_HI))

#define _ATTR_CFG_GET_FLD(attr, cfg, lo, hi) ((((attr)->cfg) >> lo) & GENMASK(hi - lo, 0))

#define ATTR_CFG_GET_FLD(attr, name) \
    _ATTR_CFG_GET_FLD(attr, ATTR_CFG_FLD_##name##_CFG, ATTR_CFG_FLD_##name##_LO, ATTR_CFG_FLD_##name##_HI)
...
static void arm_spe_event_sanitise_period(struct perf_event* event)
{
    ...
    u64 max_period = SYS_PMSIRR_EL1_INTERVAL_MASK << SYS_PMSIRR_EL1_INTERVAL_SHIFT;
...
{
    struct perf_event_attr* attr = &event->attr;
    return ATTR_CFG_GET_FLD(attr, min_latency) << SYS_PMSLATFR_EL1_MINLAT_SHIFT;
}
...
static void arm_spe_perf_aux_output_begin(struct perf_output_handle* handle, struct perf_event* event)
{
    u64 base, limit;
    ...
    limit = buf->snapshot ? arm_spe_pmu_next_snapshot_off(handle) : arm_spe_pmu_next_off(handle);
    if (limit) {
...
/* IRQ handling */
static enum arm_spe_pmu_buf_fault_action arm_spe_pmu_buf_get_fault_act(struct perf_output_handle* handle)
{
    ...
    if (pmbsr & BIT(SYS_PMBSR_EL1_DL_SHIFT)) {
        perf_aux_output_flag(handle, PERF_AUX_FLAG_TRUNCATED | PERF_AUX_FLAG_PARTIAL);
    }
    ...
    switch (pmbsr & (SYS_PMBSR_EL1_BUF_BSC_MASK << SYS_PMBSR_EL1_BUF_BSC_SHIFT)) {
...
out_err:
    pr_err_ratelimited("%s on CPU %d [PMBSR=0x%016llx, PMBPTR=0x%016llx, PMBLIMITR=0x%016llx]\n", err_str,
        smp_processor_id(), pmbsr, read_sysreg_s(SYS_PMBPTR_EL1), read_sysreg_s(SYS_PMBLIMITR_EL1));
...
static int arm_spe_pmu_event_init(struct perf_event* event)
{
    ...
    if (event->cpu >= 0 && !cpumask_test_cpu(event->cpu, &spe_pmu->supported_cpus)) {
        return -ENOENT;
    }
    ...
    if ((reg & BIT(SYS_PMSFCR_EL1_FE_SHIFT)) && !(spe_pmu->features & SPE_PMU_FEAT_FILT_EVT)) {
        return -EOPNOTSUPP;
    }
    if ((reg & BIT(SYS_PMSFCR_EL1_FT_SHIFT)) && !(spe_pmu->features & SPE_PMU_FEAT_FILT_TYP)) {
        return -EOPNOTSUPP;
    }
    if ((reg & BIT(SYS_PMSFCR_EL1_FL_SHIFT)) && !(spe_pmu->features & SPE_PMU_FEAT_FILT_LAT)) {
        return -EOPNOTSUPP;
    }
    reg = arm_spe_event_to_pmscr(event);
    if (!capable(CAP_SYS_ADMIN)
        && (reg & (BIT(SYS_PMSCR_EL1_PA_SHIFT) | BIT(SYS_PMSCR_EL1_CX_SHIFT) | BIT(SYS_PMSCR_EL1_PCT_SHIFT)))) {
        return -EACCES;
    }
    return 0;
}
...
static void* arm_spe_pmu_setup_aux(int cpu, void** pages, int nr_pages, bool snapshot)
{
    int i;
    struct page** pglist = NULL;
    struct arm_spe_pmu_buf* buf = NULL;
...
static void __arm_spe_pmu_dev_probe1(u64 reg, struct arm_spe_pmu* spe_pmu)
{
...
static void __arm_spe_pmu_dev_probe2(int fld, struct arm_spe_pmu* spe_pmu, struct device* dev)
{
    switch (fld) {
        case 0:
...
static void __arm_spe_pmu_dev_probe(void* info)
{
    ...
    fld = cpuid_feature_extract_unsigned_field(read_cpuid(ID_AA64DFR0_EL1), ID_AA64DFR0_PMSVER_SHIFT);
    if (!fld) {
    ...
    dev_info(dev, "probed for CPUs %*pbl [max_record_sz %u, align %u, features 0x%llx]\n",
...
static int arm_spe_pmu_dev_init(struct arm_spe_pmu* spe_pmu)
{
    ...
    ret = request_percpu_irq(spe_pmu->irq, arm_spe_pmu_irq_handler, DRVNAME, spe_pmu->handle);
    if (ret) {
        return ret;
    }
    /*
     ...
     * up.
     */
    ret = cpuhp_state_add_instance(arm_spe_pmu_online, &spe_pmu->hotplug_node);
    if (ret) {
...
static const struct platform_device_id arm_spe_match[] = {{ARMV8_SPE_PDEV_NAME, 0}, {}};
MODULE_DEVICE_TABLE(platform, arm_spe_match);

static int arm_spe_pmu_device_probe(struct platform_device* pdev)
{
    int ret;
    struct arm_spe_pmu* spe_pmu = NULL;
...
static void* (*__perf_aux_output_begin)(struct perf_output_handle* handle, struct perf_event* event);
...
__weak void* perf_aux_output_begin(struct perf_output_handle* handle, struct perf_event* event)
{
...
static int __init arm_spe_pmu_init(void)
{
    ...
    ret = cpuhp_setup_state_multi(CPUHP_AP_ONLINE_DYN, DRVNAME, arm_spe_pmu_cpu_startup, arm_spe_pmu_cpu_teardown);
    if (ret < 0) {
        return ret;
    }
